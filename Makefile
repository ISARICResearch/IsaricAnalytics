...
REPO := https://github.com/ISARICResearch/ISARICAnalytics

PACKAGE_NAME := isaricanalytics
BRANCH := $(shell git rev-parse --abbrev-ref HEAD)
HEAD := $(shell git rev-parse --short=8 HEAD)
PACKAGE_VERSION := $(shell grep __version__ isaricanalytics/__init__.py | cut -d '=' -f 2 | xargs)
...
clean:
	...
	python3 -m pip uninstall -y isaricanalytics
...
.PHONY: sdist
sdist: clean
	@echo "$(PACKAGE_NAME)[$(BRANCH)@$(HEAD)]: Building a source distribution of ISARICAnalytics"
	uv build --verbose --sdist && ls -al dist/*.tar.gz

.PHONY: wheel
wheel: clean
	@echo "$(PACKAGE_NAME)[$(BRANCH)@$(HEAD)]: Building a wheel of ISARICAnalytics"
	uv run hatchling build --target wheel --clean
	ls -al dist/isaric*.whl

.PHONY: all
all: clean
	@echo "$(PACKAGE_NAME)[$(BRANCH)@$(HEAD)]: Building a source distribution + wheel of ISARICAnalytics"
...
.PHONE: uninstall
uninstall: clean
	@echo "$(PACKAGE_NAME)[$(BRANCH)@$(HEAD)]: Uninstalling any existing installed ISARICAnalytics distribution"
