...
def test_categorical_onehot_roundtrip():
    ...
    encoded = convert_categorical_to_onehot(
        df.copy(), dictionary, categorical_columns=["cat"]
    )
    assert "cat___a" in encoded.columns
    assert "cat___b" in encoded.columns

    decoded = convert_onehot_to_categorical(
        encoded.copy(), dictionary, categorical_columns=["cat"]
    )
