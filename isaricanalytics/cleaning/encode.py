...
def collapse_options(
    ...
    Args:
        ...
        cumulative_threshold:
            ...
            values) are retained and all else are replaced with :replacement_string:.
        max_n_options:
            Threshold for collapsing values into :replacement_string:.
            A value of `n` means that the `n` most frequent categories are retained
            and all else are replaced with :replacement_string:.
        replacement_string:
    ...
    """
    if cumulative_threshold and (cumulative_threshold < 0 or cumulative_threshold > 1):
...
def one_hot_encode(
    ...
    if not hasattr(data, table_name):
        logger.info(
    ...
    categorical_field_names = data.get_field_names(
    ...
    if collapse_to_other and (cumulative_threshold or max_n_options):
        for field_name in valid_field_names:
            # TODO: check skip_logic_filter function
            # Keep rows where skip logic is satisfied
            mask = filters.skip_logic_filter(
                data=data,
                field_name=field_name,
            )
            df.loc[mask, field_name] = collapse_options(
